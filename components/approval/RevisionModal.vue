  ...
  >
    ...
    <div class="space-y-4 py-2">
      <div class="space-y-1.5">
        ...
        ></textarea>
        <p v-if="error" class="text-[11px] font-semibold text-rose-600 dark:text-rose-400">
          {{ error }}
        </p>
      </div>
      ...
      <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/60 text-xs text-slate-500 space-y-1">
        <p class="font-bold text-slate-700 dark:text-slate-300">What happens next?</p>
        <p class="leading-relaxed">
          The evaluation status will change to <strong class="text-rose-600 dark:text-rose-400">REVISION_REQUIRED</strong>. The supervisor will be notified to adjust score, comment, or evidence and resubmit for review.
        </p>
      </div>
    </div>
    ...
    <template #footer>
      ...
      <button
        type="button"
        @click="handleConfirm"
        class="inline-flex items-center gap-2 px-5 py-2 text-xs font-bold rounded-xl bg-rose-600 hover:bg-rose-700 text-white shadow-md shadow-rose-600/20 transition-all active:scale-95"
      >
