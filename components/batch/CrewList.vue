    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center justify-between">
              ...
              <span class="text-[10px] font-bold px-1.5 py-0.2 rounded bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-300">
                Lvl {{ crew.level }}
              </span>
            </div>
            <h4 class="text-xs font-bold text-slate-900 dark:text-white truncate mt-0.5">
              {{ crew.name }}
            </h4>
            <p class="text-[11px] text-slate-400 dark:text-slate-500 truncate">
              {{ crew.position }}
            </p>
            <p class="text-[10px] font-semibold text-[#499ec7] dark:text-[#84cded] truncate mt-0.5">
              📍 {{ crew.storeLocation }}
            </p>
          </div>
        </div>
        ...
        <div class="mt-4 pt-3 border-t border-slate-200/60 dark:border-slate-800/60 flex items-center justify-between text-xs">
          ...
          <div class="inline-flex items-center gap-1 font-extrabold text-amber-600 dark:text-amber-400 text-xs">
        ...
        <div v-if="userStore.isSupervisor" class="mt-3 pt-2">
          <NuxtLink
            ...
            class="w-full flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl text-[11px] font-bold bg-[#499ec7] hover:bg-[#24779f] text-white shadow-sm transition-all"
          >
