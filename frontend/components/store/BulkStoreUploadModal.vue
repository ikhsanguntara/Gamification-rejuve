        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-base font-bold text-slate-900 dark:text-white">
              Bulk Update Master Gerai (Excel)
            </h3>
            <p class="text-xs text-slate-500 dark:text-slate-400">
              Perbarui penugasan Store Leader, District Manager, dan status operasional gerai secara massal via Excel (.xlsx).
            </p>
          </div>
        </div>
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                <h4 class="text-xs font-bold text-amber-900 dark:text-amber-300">
                  Gunakan Template Excel Pre-Populated Resmi
                </h4>
                <p class="text-[11px] text-amber-700/90 dark:text-amber-400/90 mt-0.5">
                  Template ini otomatis memuat seluruh daftar gerai terdaftar lengkap dengan dropdown pilihan Store Leader & District Manager aktif.
                </p>
              </div>
            </div>
            ...
            <div v-else class="flex items-center gap-4 text-left w-full max-w-md p-3.5 bg-white dark:bg-slate-800 rounded-xl border border-emerald-500/30 shadow-xs">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-[11px] text-slate-400">
                  {{ formatBytes(selectedFile.size) }} • Siap divalidasi
                </p>
              </div>
              <button
                type="button"
        ...
        <div v-if="currentStep === 2" class="space-y-4 animate-in fade-in-50 duration-150">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div class="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200/80 dark:border-emerald-800/80 flex items-center justify-between">
              <div>
                <p class="text-[11px] font-semibold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Siap Diperbarui</p>
          ...
          <div v-if="previewSummary.invalidCount > 0" class="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-2.5 text-xs text-amber-800 dark:text-amber-300">
            ...
            <span>
              Terdapat <strong>{{ previewSummary.invalidCount }} data tidak valid</strong>. Baris yang memiliki kesalahan akan dilewati saat proses commit, atau Anda dapat memperbaiki file Excel dan mengunggah kembali.
            </span>
          </div>
          ...
          <div class="flex items-center justify-between gap-3 flex-wrap">
            ...
            <span class="text-xs text-slate-400">
              Menampilkan {{ filteredPreviewRows.length }} dari {{ previewSummary.total }} gerai
            </span>
          </div>
            ...
            <table class="w-full text-left text-xs">
              <thead class="bg-slate-50 dark:bg-slate-800/80 sticky top-0 border-b border-slate-200 dark:border-slate-700 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                <tr>
                  ...
                  <th class="py-2.5 px-3">Kode Gerai</th>
                  <th class="py-2.5 px-3">Nama Gerai</th>
                  <th class="py-2.5 px-3">Wilayah</th>
                  <th class="py-2.5 px-3">Store Leader (SL)</th>
                  <th class="py-2.5 px-3">District Manager (DM)</th>
                  <th class="py-2.5 px-3">Status Operasional</th>
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                >
                  ...
                  <td class="py-2.5 px-3 font-mono font-bold text-slate-800 dark:text-slate-200">{{ row.departmentCode || '—' }}</td>
                  <td class="py-2.5 px-3 font-semibold text-slate-900 dark:text-white">{{ row.departmentName || '—' }}</td>
                  <td class="py-2.5 px-3 text-slate-500 dark:text-slate-400 text-[11px]">{{ row.regionCode || '—' }}</td>
                  <td class="py-2.5 px-3">
                    <div v-if="row.slEmail || row.slName" class="flex flex-col">
                      <span v-if="row.slName" class="font-semibold text-slate-800 dark:text-slate-200 text-xs">
                        {{ row.slName }}
                      </span>
                      <span v-if="row.slEmail" class="text-[11px] text-slate-500 dark:text-slate-400 font-mono">
                        {{ row.slEmail }}
                      </span>
                    </div>
                  ...
                  <td class="py-2.5 px-3">
                    <div v-if="row.dmEmail || row.dmName" class="flex flex-col">
                      <span v-if="row.dmName" class="font-semibold text-slate-800 dark:text-slate-200 text-xs">
                        {{ row.dmName }}
                      </span>
                      <span v-if="row.dmEmail" class="text-[11px] text-slate-500 dark:text-slate-400 font-mono">
                        {{ row.dmEmail }}
                      </span>
                    </div>
                    <span v-else class="text-slate-400 italic text-[11px]">Belum Ditugaskan</span>
                  ...
                  <td class="py-2.5 px-3">
                    <span
                      ...
                      :class="row.isActive ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-slate-100 text-slate-500'"
                    >
                  ...
                  <td class="py-2.5 px-3 text-center">
                    ...
                    <div v-else class="inline-flex flex-col items-center">
                      ...
                      <span v-if="row.errorMessage || row.error" class="text-[9px] text-rose-500 mt-0.5 max-w-[140px] truncate" :title="row.errorMessage || row.error">
        ...
        <div v-if="currentStep === 3" class="py-8 text-center space-y-4 animate-in zoom-in-95 duration-200">
          ...
          <div class="max-w-md mx-auto space-y-1">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white">
              Pembaruan Master Gerai Berhasil!
            </h3>
            <p class="text-xs text-slate-500 dark:text-slate-400">
              Sebanyak <strong>{{ commitResult?.updatedCount || previewSummary.validCount }} gerai / departemen</strong> telah berhasil diperbarui ke dalam database Re.juve.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="px-5 py-3.5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/40 flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2.5">
          ...
          >
            ...
            <span>{{ isCommitting ? 'Menyimpan ke Database...' : `Commit & Perbarui (${previewSummary.validCount} Gerai)` }}</span>
          </button>
...
async function handleDownloadTemplate() {
  ...
  try {
    await storeStore.downloadTemplate()
    toast.success('Download Berhasil', 'Template Excel Gerai (.xlsx) siap diisi.')
  } catch (err) {
    toast.error('Gagal Mengunduh Template', err.message || 'Terjadi kesalahan saat mengunduh template Excel gerai.')
  } finally {
...
async function handleRunPreview() {
  ...
  try {
    const res = await storeStore.previewBulkDepartments(selectedFile.value)
    ...
    if (Array.isArray(res)) {
      ...
      rows = res.departments
    } else if (Array.isArray(res?.preview)) {
      rows = res.preview
    }

    // Map each row with robust fallbacks
    previewRows.value = rows.map((r, i) => {
      const slEmail = r.emailSl || r.slEmail || r.storeLeaderEmail || r.emailStoreLeader || ''
      const slName = r.userSlName || r.slName || r.storeLeaderName || r.nameSl || ''
      const userSlId = r.userSlId || r.slId || null

      const dmEmail = r.emailDm || r.dmEmail || r.districtManagerEmail || r.emailDistrictManager || ''
      const dmName = r.userDmName || r.dmName || r.districtManagerName || r.nameDm || ''
      const userDmId = r.userDmId || r.dmId || null

      return {
        rowNumber: r.rowNumber || r.row || i + 1,
        departmentCode: r.departmentCode || r.code || '',
        departmentName: r.departmentName || r.name || '',
        regionCode: r.regionCode || r.region || r.cityCode || '—',
        slEmail,
        slName,
        userSlId,
        dmEmail,
    ...
    toast.error('Gagal Validasi Excel', err.message || 'Terjadi kesalahan saat memproses preview bulk update gerai.')
  } finally {
  ...
  try {
    const validRows = previewRows.value
      ...
      .map(r => ({
        ...
        regionCode: r.regionCode !== '—' ? r.regionCode : null,
        emailSl: r.slEmail || null,
        slEmail: r.slEmail || null,
        userSlId: r.userSlId || null,
        emailDm: r.dmEmail || null,
        dmEmail: r.dmEmail || null,
        userDmId: r.userDmId || null,
        isActive: Boolean(r.isActive)
      }))

    const payload = {
      departments: validRows
    }

    const res = await storeStore.commitBulkDepartments(payload)
    ...
    emit('imported', res)
    toast.success('Pembaruan Gerai Selesai', `Berhasil memperbarui ${validRows.length} gerai ke database.`)
  } catch (err) {
    toast.error('Gagal Commit Data', err.message || 'Terjadi kesalahan saat memperbarui data ke database.')
  } finally {
