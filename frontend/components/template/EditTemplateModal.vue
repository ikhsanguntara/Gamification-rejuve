<template>
  ...
  >
    <form @submit.prevent="executeSaveAll" class="space-y-5 py-2">
      ...
      <!-- ========================================== -->
      <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3 flex-wrap">
        <p class="text-[11px] text-slate-400">
          * Seluruh konfigurasi header dan {{ form.details.length }} butir misi akan dikirim serentak ke API.
        </p>
        ...
        <div class="flex items-center gap-3">
          ...
          >
            ...
            <span>{{ isSubmitting ? 'Menyimpan ke Server...' : 'Simpan Seluruh Perubahan (Header + Details)' }}</span>
          </button>
        </div>
      </div>
    </form>
...
watch(() => props.modelValue, (isOpen) => {
  if (isOpen && props.template) {
    ...
    form.value = {
      ...
      durationValue: Number(props.template.durationValue || props.template.totalWeeks || 3),
