...
const avatarPreview = computed(() => {
  return form.value.avatarUrl || form.value.avatar || getRandomAvatar(form.value.name || user.value?.id || 'Crew')
})
  ...
  (u) => {
    if (u) {
      form.value = {
        ...
        userBuddyId: u.userBuddyId || null,
        avatarUrl: u.avatarUrl || u.avatar || '',
        avatar: u.avatarUrl || u.avatar || getRandomAvatar(u.name || u.id),
        isBuddy: Boolean(u.isBuddy)
      }
    }
  },
...
const handleUpdate = async () => {
  ...
  try {
    ...
    const finalAvatar = form.value.avatarUrl || form.value.avatar || getRandomAvatar(form.value.name || user.value.id)
