  ...
  <div class="space-y-6 pb-16">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <div class="flex items-center gap-2.5 mb-1.5 flex-wrap">
          <div class="w-9 h-9 rounded-2xl bg-gradient-to-br from-amber-600 to-orange-700 text-white flex items-center justify-center font-bold shadow-md shadow-amber-600/20">
            <ShieldCheck class="w-5 h-5" />
          </div>
          <h2 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">
            Report by DM (Supervisi DM)
          </h2>
        ...
        <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-3xl">
          Evaluasi kecepatan approval District Manager, akumulasi perolehan poin, dan pemantauan kartu misi belum dinilai.
        </p>
      </div>
      ...
      <div class="flex items-center gap-2.5 flex-wrap">
        <button
          ...
          title="Download Spreadsheet Excel Report by DM (.xlsx)"
        >
        ...
        <div class="lg:col-span-2">
          ...
          <div class="relative">
            ...
            <input
              v-model="searchQuery"
              type="text"
              placeholder="Cari kode gerai / nama gerai / nama DM..."
              @input="onSearchInput"
    ...
    <div class="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs overflow-hidden">
      <div class="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
        <div>
          <h3 class="font-bold text-base text-slate-900 dark:text-white">
            Report by DM (Evaluasi Supervisi & Approval DM)
          </h3>
          <p class="text-xs text-slate-500 dark:text-slate-400">
            Kecepatan approval District Manager, akumulasi poin persetujuan, dan pengawasan kartu misi yang belum dinilai.
          </p>
        </div>
        <span class="text-xs font-semibold px-3 py-1 rounded-full bg-amber-50 dark:bg-amber-950/80 text-amber-700 dark:text-amber-300">
          Total: {{ filteredDmList.length }} Gerai Supervisi
        </span>
      </div>
      ...
      <div v-if="reportStore.isLoading" class="p-12 text-center">
        ...
        <p class="text-sm font-semibold text-slate-600 dark:text-slate-400">Memuat data supervisi DM...</p>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredDmList.length === 0" class="p-12 text-center">
        <ShieldCheck class="w-12 h-12 text-slate-300 dark:text-slate-600 mx-auto mb-3" />
        <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">Tidak ada data report DM.</p>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
              ...
              <th class="py-3.5 px-4">Kode & Gerai</th>
              <th class="py-3.5 px-4">District Manager</th>
              <th class="py-3.5 px-4 text-center">Rata-rata Respon Approval</th>
              <th class="py-3.5 px-4 text-center">Jml Batch</th>
              <th class="py-3.5 px-4 text-center">New Hire</th>
              <th class="py-3.5 px-4 text-center">Poin Diberikan</th>
              <th class="py-3.5 px-4 text-center text-rose-500">Unscored SL</th>
              <th class="py-3.5 px-4 text-center text-amber-500">Unscored DM</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
            <tr
              v-for="(item, idx) in filteredDmList"
            ...
            >
              ...
              <td class="py-3.5 px-4 text-center font-medium">
                <span class="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-[11px] text-slate-700 dark:text-slate-300 inline-flex items-center gap-1">
                  <Clock class="w-3 h-3 text-slate-400" />
                  {{ item.avgApprovalFormatted || (item.avgApprovalHours ? `${item.avgApprovalHours} Jam` : '-') }}
                </span>
              </td>
              ...
              <td class="py-3.5 px-4 text-center font-black text-amber-600 dark:text-amber-400 font-mono">
                💎 {{ (item.totalPointsGiven || 0).toLocaleString('id-ID') }}
              </td>
              <td class="py-3.5 px-4 text-center font-bold text-rose-600 dark:text-rose-400">
                {{ item.unscoredMissionsBySl || 0 }}
              </td>
              <td class="py-3.5 px-4 text-center font-bold text-amber-600 dark:text-amber-400">
                {{ item.unscoredMissionsByDm || 0 }}
              </td>
            </tr>
          </tbody>
        </table>
...
const loadData = async () => {
  ...
  await reportStore.fetchDmReport(customParams)
}
...
const refreshData = async () => {
  await loadData()
  toast.success('Data Diperbarui', 'Data supervisi DM berhasil dimuat ulang.')
}

const filteredDmList = computed(() => {
  let list = reportStore.dmReports || []
  const q = searchQuery.value.trim().toLowerCase()
  if (q) {
    list = list.filter(d =>
      (d.storeName && d.storeName.toLowerCase().includes(q)) ||
      (d.storeCode && d.storeCode.toLowerCase().includes(q)) ||
      (d.dm && d.dm.toLowerCase().includes(q))
    )
  }
  return list
})
...
const handleExport = async () => {
  try {
    await reportStore.exportDmReport()
    toast.success('Ekspor Berhasil', 'Spreadsheet Report by DM (.xlsx) telah diunduh.')
  } catch (err) {
